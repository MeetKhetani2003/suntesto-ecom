...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (tokenResponse.ok) {
      ...
      console.warn('Google Token exchange returned error. Falling back to simulated login mode for local sandbox testing.');
    }
  } catch (error) {
    console.error('Google OAuth API error, falling back to simulated sandbox profile:', error);
  }

  try {
    // 3. Upsert User profile inside MongoDB
